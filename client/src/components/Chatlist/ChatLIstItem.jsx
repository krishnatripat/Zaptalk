  ...
  return (<div className={`flex cursor-pointer items-center hover:bg-background-default-hover`} onClick={handleContactClick}>
    ...
    <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
      <div className="flex justify-between ">
        ...
        {!isContactPage && (
          <div><span className={`${!data.totalUnreadMessages > 0 ? "text-secondary" : "text-icon-green"} text-sm`}>
            {calculateTime(data.createdAt)}</span></div>
        )}
      </div>
      <div className="flex border-b border-conversation-border pb-2 pt-1 p">
        <div className="justify-between flex w-full ">
          ...
          {data.totalUnreadMessages>0 &&<span className="bg-icon-green px-[5px] roundedfull text-sm">{data.totalUnreadMessages}</span>}
        </div>
      </div>
    </div>
  </div>
